	...
	public List<Absensi_mobile> get_attendance(String id_pegawai, String api_key){
		...
		if(id_app == id_aplikasi){
			if(api_key.equals(api_keydb)){
				...
				List<Map<String, Object>> absRows = jdbcTemplate.queryForList("SELECT * FROM `oasys_attendance_log` WHERE month(date_time) = month(now()) AND year(date_time) = year(now()) AND id_pegawai=?", id_pegawai);
